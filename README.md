...
Последнее изменение: 13.12.2025 19:13
...
1. Убедитесь, что скрипт `post-commit` находится в директории `scripts/`
...
3. Создайте символическую ссылку на скрипт в директории `.git/hooks/`:

   ```bash
   ln -sf ../../scripts/post-commit .git/hooks/post-commit
   ```

После этого при каждом коммите дата в строке "Последнее изменение:" в README.md будет автоматически обновляться.
